...
func NewFs(name, root string, m configmap.Mapper) (fs.Fs, error) {
	...
	fs.Debugf(f, "Root id: %s", f.dirCache.RootID())
	return f, nil
}
...
func (f *Fs) List(ctx context.Context, dir string) (entries fs.DirEntries, err error) {
	fs.Debugf(f, "--- List: %s %s", f.root, dir)
	err = f.dirCache.FindRoot(ctx, false)
	if err != nil {
		return nil, err
	}
	...
	for _, child := range children {
		fs.Debugf(f, "child: %s", child.Name)
		remote := path.Join(dir, child.Name)
		fs.Debugf(f, "child remote: %s", remote)
		if child.IsDir() {
...
func (f *Fs) createUpload(ctx context.Context, name string, size int64, parentID string, modTime time.Time) (location string, err error) {
	fs.Debugf(f, "creating upload. name: %s, parent_id: %s", name, parentID)
	...
	fs.Debugf(f, "modtime: %s", modTime.Format(time.RFC3339))
	...
	if resp.StatusCode != 201 {
		return "", fmt.Errorf("unexpected status code from upload create: %d", resp.StatusCode)
	}
	loc := resp.Header.Get("location")
	if loc == "" {
		return "", errors.New("empty location header from upload create")
	}
	return loc, nil
}

func (f *Fs) sendUpload(ctx context.Context, location string, size int64, in io.Reader) (fileID int64, err error) {
	fs.Debugf(f, "sending upload to location: %s", location)
	if size == 0 {
		err = f.pacer.Call(func() (bool, error) {
...
func (f *Fs) transferChunk(ctx context.Context, location string, start int64, chunk io.ReadSeeker, chunkSize int64) (int64, error) {
...
func (f *Fs) Mkdir(ctx context.Context, dir string) error {
	...
	fs.Debugf(f, "mkdir: found root: %s", f.dirCache.RootID())
	if dir != "" {
...
func (f *Fs) Copy(ctx context.Context, src fs.Object, remote string) (fs.Object, error) {
	fs.Debugf(f, "--- Copy: %s to %s", src.Remote(), remote)
...
func (f *Fs) Move(ctx context.Context, src fs.Object, remote string) (fs.Object, error) {
	fs.Debugf(f, "--- Move: %s to %s", src.Remote(), remote)
	srcObj, ok := src.(*Object)
	if !ok {
...
func (f *Fs) DirMove(ctx context.Context, src fs.Fs, srcRemote, dstRemote string) error {
	fs.Debugf(f, "--- DirMove src: %s dst: %s", srcRemote, dstRemote)
	srcFs, ok := src.(*Fs)
	if !ok {
